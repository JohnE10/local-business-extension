...
import { deleteKeyValue, findValueInObjArr, isValidUrl, stringContains } from '../../utils/helpers';
                                    ...
                                    if (!findValueInObjArr(toBeUpdatedInDB, 'tempId', tempId)) { // check for dupes
                                        ...
                                        if (!await Business.findOne({ email: objArr[i]['email'] })) {
                                            ...
                                            headers.map((header) => {
                                                if (objArr[i][header]) {
                                                    if (objArr[i][header] == '') {
                                                        tempObj[header] = document[header];
                                                    }
                                                    else {
            ...
            if (toBeUpdatedInDB) {
                if (toBeUpdatedInDB.length > 0) {
                    toBeUpdatedInDB.map(async (ele) => {
                        if (!stringContains(ele['email'], excludeUrls)) {
                            await Business.updateOne({ _id: ele['tempId'] }, { $set: { email: ele['email'] } });
                        }
                        else {
                            excludedEmails.push(ele);
                        }
                    })
                }
            }
